...
- **Decision.** Where monitored patterns live, and whether the per-decision
  flag belongs on the customer read path at all — the periodic sampler of §7
  reads the same attributes from the backoffice side, where the audience is
  already right.

## 3. Narrow the customer read path's session lookup
...
## 4. One source of truth for the policy thresholds
...
## 5. Policy retrieval and the citations it would produce
...
## 6. Collect and check the required documents
...
## 7. Fair-lending review producer
...
## 8. Verify the load balancer's hop to PAF
...
## 9. Select AI as the cloud tool transport — on standby
...
### 10.1 Deterministic `upsert` via marker — only if needed
...
### 10.2 PL/SQL Executor node — safe in-DB calls (alternative for `issues/02`)
...
### 10.3 Agent observability / OTel tracing — mitigates `issues/04` and `issues/08`
