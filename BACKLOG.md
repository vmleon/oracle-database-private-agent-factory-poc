...
in their own right — §3 and §9 — and the profile itself has never been created,
...
- **Decision.** Select AI Bridge, which waits on §3 and §9, or a read-only MCP
...
## 4. Let a reviewer claim from the queue
...
## 5. Complete the decision record
...
## 6. Tell the customer the outcome
...
## 7. Run the compliance checks that are already served
...
## 8. One source of truth for the policy thresholds
...
## 9. Policy retrieval and the citations it would produce
...
## 10. Collect and check the required documents
...
## 11. Narrow the customer read path's session lookup
...
## 12. Similar-case lookup for the reviewer
...
## 13. Fair-lending review producer

`BANK_CORE.fair_lending_review` was created with the config schema and is never written or read, and the per-decision fair-lending flag is uncalled (§7), so neither the pre-flight check nor the periodic review runs.
