...
The `decision` Blockchain row is meant to be the whole record of a bank decision. The insert copies the human's call and the agent packet across and leaves four columns null — `pricing_offer`, `reason_codes`, `computed_dti`, `computed_pti` — and the reviewer's decision-detail view reads all four. Reason codes survive inside the evidence JSON; the two ratios are lost even though the same turn computed them.

It leads the backlog because it is the only item with a deadline. `BANK_CORE.decision` is `NO DELETE LOCKED` and holds no rows yet, so nothing is lost — but the first closed task writes a permanently incomplete record, and every one after it does the same.
...
## 2. Stand up `RESEARCH_WORKFLOW`, the backoffice research agent
...
## 3. Let a reviewer claim from the queue
...
## 4. Tell the customer the outcome
...
## 5. Run the compliance checks that are already served
...
It lands as a read-only tool on `RESEARCH_WORKFLOW`, so it follows §2.

## 11. Fair-lending review producer

`BANK_CORE.fair_lending_review` was created with the config schema and is never written or read, and the per-decision fair-lending flag is uncalled (§5), so neither the pre-flight check nor the periodic review runs.
