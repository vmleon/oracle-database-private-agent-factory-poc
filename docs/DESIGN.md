...
- **`CHAT_WORKFLOW`** — customer-facing. Reads only the customer-safe `REPORTING.*` view set, runs OPA, writes a recommendation packet to `hitl_task` via the `create_hitl_task` in-DB tool. Cannot write to `decision`.
...
Heavy data work — SQL over banking views, vector search over `policy_corpus` and `case_history` — runs **in-database** via Select AI profiles, tasks, tools, and teams. Policy and side-effect work — OPA evaluation and HITL task creation — runs in **near-DB MCP/REST services** and is wired to `CHAT_WORKFLOW` only.
...
- **MCP Server nodes**: `opa-mcp` — registered, but wired only to `CHAT_WORKFLOW`.
- **Agent Builder flows** (two):
  - `CHAT_WORKFLOW`: a four-agent origination pipeline — `Concierge` (conversational intake: `banking-mcp.get_context` + `application-mcp.upsert_application`) → `Docs & Employer` (`required_documents` + Company Registry `verify_employer`) → `Eligibility` (`evaluate_eligibility`) → `Recommendation` (HITL MCP `create_hitl_task` only — the workflow's single side-effect tool). Every agent re-reads state from `banking-mcp.get_context` ("the database is the memory"); the split keeps each agent to ≤3 tool calls under PAF's hardcoded 5-iteration cap. Full build blueprint: [`paf/flows/CHAT_WORKFLOW.md`](../paf/flows/CHAT_WORKFLOW.md).
...
| Side-effects                           | `CHAT_WORKFLOW`'s only side-effect tool is `create_hitl_task`, plus OPA MCP calls. `RESEARCH_WORKFLOW` is read-only — no write tools, no enqueue. Enforced by `AGENT_TOOLS` grants and by which MCP servers are wired to which flow.                        |
...
- **Two agents in PAF, distinct tool scopes.** The platform configures `CHAT_WORKFLOW` (customer-facing, OPA + Select AI over the customer-safe view set + Company Registry HTTP datasource + `create_hitl_task`) and `RESEARCH_WORKFLOW` (backoffice-only, Select AI over a broader read-only view set + RAG, read-only). Same factory, two security envelopes.
...
- **`CHAT_WORKFLOW` and `RESEARCH_WORKFLOW` shape**: both are explicit Agent Builder **DAGs** (directed acyclic graphs of nodes — Prompt, Agent, Parser, Condition, Tool, Chat Output — wired together with no cycles, so each run has a well-defined path through the flow). `CHAT_WORKFLOW` is a **four-agent origination pipeline**: Chat Input → RegexExtractor (token) → `Concierge` (intake) → `Docs & Employer` → `Eligibility` → `Recommendation` (one side-effect tool, `create_hitl_task`), with `Condition` gates between agents. The split keeps each agent to ≤3 tool calls under PAF's hardcoded 5-iteration cap and isolates the side effect — see [`paf/flows/CHAT_WORKFLOW.md`](../paf/flows/CHAT_WORKFLOW.md). `RESEARCH_WORKFLOW` is intentionally smaller: Prompt → Agent → Chat Output, no side-effect nodes.
