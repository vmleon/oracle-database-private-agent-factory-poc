...
- **Generative model**: **`Qwen/Qwen2.5-72B-Instruct-AWQ`** served by vLLM. AWQ 4-bit quant (~40 GB resident), native tool-call support via `--tool-call-parser=hermes`. Chosen over the smaller `Qwen/Qwen2.5-32B-Instruct-AWQ` after the latter proved borderline on the 4-tool `CHAT_WORKFLOW` recipe — 32B-AWQ occasionally leaked chat-template tokens (`<|im_start|>`, `<tool_response>`) into assistant text and fabricated inline tool responses, eating iterations against PAF's `max_iterations=5` cap. 72B AWQ is comfortable on DGX Spark's 128 GB unified memory with headroom for KV cache + the embedding model. Step up to `Qwen/Qwen2.5-72B-Instruct-FP8` (~75 GB) for higher quality if GPU memory allows. Swap by editing `VLLM_GEN_MODEL` in `.env` and `docker compose down && up -d` on the vLLM stack — no code change needed.
