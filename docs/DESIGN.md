...
- **Application Service** persists applications, owns document upload, runs cheap OPA pre-checks, and invokes the agents. It exposes two distinct PAF surfaces: `/chat/*` for the customer `CHAT_FLOW` and `/research/*` for the backoffice `RESEARCH_WORKFLOW` — `POST /v1/research/tasks/{taskId}/run` starts a research run for a task and screens its summary before storing it, and `GET /v1/research/tasks/{taskId}` returns the stored summary so reopening a case does not re-run the agent. `research-mcp` posts each tool call to `POST /v1/audit/research`, which the Application Service writes to `research_audit`.
...
- **`RESEARCH_WORKFLOW`** — backoffice-only. Reads a **broader, read-only** scope through `research-mcp` (full transaction history, `decision_audit`, `policy_parameter_history`, banded similarity over `case_history`). Has **no side-effect tools** — it cannot create HITL tasks, cannot record decisions, cannot mutate state of any kind.
...
| Per-tool audit envelope                                     | Each MCP wrapper posts its call to the Application Service, which writes `decision_audit` (`CHAT_FLOW`) or `research_audit` (`RESEARCH_WORKFLOW`) | Both                | MCP wrapper → Application Service |
| Customer profile / transactions / bureau queries            | Select AI profile + tasks over curated views                                                                                                      | `CHAT_FLOW`         | In-DB                             |
| Broader read scope (audit, parameter history, transactions) | `research-mcp` `*_for_task` tools, deterministic MCP reads                                                                                        | `RESEARCH_WORKFLOW` | Near-DB                           |
| Comparable-case similarity                                  | `research-mcp` banded match over `case_history` (amount / DTI / score windows)                                                                    | `RESEARCH_WORKFLOW` | Near-DB                           |
| Policy citations                                            | Select AI RAG tool over Oracle AI Vector Search                                                                                                   | `CHAT_FLOW`         | In-DB                             |
...
| `src/ai/`            | Python                | The three FastMCP wrappers PAF calls as MCP servers, one per database identity: `banking-mcp` (the deterministic `*_for_session` reads, the OPA calls and the tier rule in `gate.py`; `CUSTOMER_AGENT_RO`), `application-mcp` (`upsert_application` and `create_hitl_task`; `CUSTOMER_AGENT_RW`) and `research-mcp` (five read-only `*_for_task` tools — `recommendation_for_task`, `similar_cases_for_task`, `decision_history_for_task`, `transactions_for_task`, `policy_changes_for_task`; `BACKOFFICE_AGENT_RO`, `SELECT` only). systemd units on the `backend` compute behind the internal TLS load balancer. |
...
Database identities come in two kinds, and the split is what bounds a compromise. **Owners** hold the objects and are created without `CREATE SESSION`, so nothing can log in as them and a leaked owner password opens nothing. **Clients** log in, own nothing, and carry only the grants their consumer needs. Privilege follows the _audience_ that can reach a client: the customer-facing agent and the backoffice one never share a login, so a prompt injection against the chat agent is confined by what `CUSTOMER_AGENT_RO` is allowed to select. The whole grant matrix is one file — `database/liquibase/020-client-grants.yaml`.
...
- **Data sources**:
  - Two **Database** data sources over `BANK_VIEWS`, connecting as `CUSTOMER_AGENT_RO` and `BACKOFFICE_AGENT_RO` respectively — the connection user is what bounds a compromised flow, so the two audiences never share a login (§6.2). Registered so Select AI's database list is populated; neither flow's graph reads through either one (§11).
...
- **Select AI profiles**: `chat_profile` — NL2SQL object list scoped to the customer-safe `BANK_VIEWS.*` views, RAG vector index over `policy_corpus`. `RESEARCH_WORKFLOW` has no Select AI profile of its own — its read scope reaches the database through `research-mcp` instead (§11).
- **MCP servers**: `banking-mcp`, `application-mcp` and `research-mcp` — one per database identity, registered by name (`manage.py paf link-flow` rebinds each flow's nodes to the install's ids). `banking-mcp` and `application-mcp` are wired only to `CHAT_FLOW`; `research-mcp` is wired only to `RESEARCH_WORKFLOW`. A server is split by the identity it logs in as, never by tool: which tool a worker sees is its MCP Server node's _Allowed MCP tools_ list, which privilege the call carries is the server's client user.
- **Agent Builder flows** (two):
  ...
  - `RESEARCH_WORKFLOW`: Chat Input → Regex extractor (splits the task id out of the `[[TASK <id>]]` envelope) → Prompt (JSON-wrap) → Type Convert → five deterministic `research-mcp` nodes off one wired task id (`recommendation_for_task`, `similar_cases_for_task`, `decision_history_for_task`, `transactions_for_task`, `policy_changes_for_task`) → a gate on whether the task resolved → Prompt (research) → one Agent node holding no tools and no sub-agents, which writes a four-section case file → Chat Output. No Select AI Bridge, no RAG, no write tool anywhere on the path — `BACKOFFICE_AGENT_RO` holds no `EXECUTE` grant and no write grant of any kind, so the flow is structurally unable to decide anything. Full build blueprint: [`paf/flows/RESEARCH_WORKFLOW.md`](../paf/flows/RESEARCH_WORKFLOW.md).
- **Integration API keys**, one per flow, minted by `manage.py paf api-key` into `.env` and handed to the backend tier: `PAF_AGENT_ID` / `PAF_API_KEY` for the published `CHAT_FLOW`, `PAF_RESEARCH_AGENT_ID` / `PAF_RESEARCH_API_KEY` for the published `RESEARCH_WORKFLOW`.
...
12. A backoffice reviewer claims the task with `DEQONE` (atomic with `hitl_task` OPEN → IN_REVIEW), reads the recommendation + reasoning + evidence, may run `RESEARCH_WORKFLOW` from the task detail screen to dig deeper — its case file passes `ResearchSummary.screen` and, once it does, is appended to `BANK_CORE.research_summary` so reopening the case does not re-run the agent — then submits a final decision. The Application Service writes the **`decision` Blockchain Table row** at task close — one row per bank decision, carrying both the human's final outcome and the original recommendation packet — and appends the customer-facing outcome to the customer's `chat_message` thread as fixed text under the disclosure policy — the outcome, never a figure or a reason code. The next time the customer opens the chat, the outcome is waiting at the end of the conversation.
...
1. **Per-tool audit** (`decision_audit`, `research_audit`) — every `CHAT_FLOW` tool call's input, output, duration, status, ordered by `agent_run_id` + `step_no`. `RESEARCH_WORKFLOW` runs are audited separately, into `research_audit`, so research conversations don't pollute the decisioning trail: `research-mcp` posts each call to `POST /v1/audit/research`, and the Application Service writes the row, ordered by `research_run_id` + `step_no`.
...
3. **Case research record** (`research_summary`) — Blockchain Table, one row per completed `RESEARCH_WORKFLOW` run. Carries the screened case file, the reviewer and the `research_run_id` that ties it back to its rows in `research_audit`. `ResearchSummary.screen` runs before the row is written, so a stored summary is either the organised case file or `ResearchSummary.BLOCKED` — never a stated verdict.
4. **Parameter history** (`policy_parameter_history`) — versioned `system_config` edits with reason. Pair with the audit view to interpret a past decision against its then-current parameters.
...
| Read-scope guardrail (`RESEARCH_WORKFLOW`) | `BACKOFFICE_AGENT_RO` holds `SELECT` only on the `research_v_*` view set and `cust_360`, no `EXECUTE` anywhere and no write grant of any kind; `research-mcp`'s five tools each take only a task id and resolve it server-side. Read-only end to end: no base-table grants.                                                                                                                                                                                                          |
...
- **Two agents in PAF, distinct tool scopes.** The platform configures `CHAT_FLOW` (customer-facing, OPA + Select AI over the customer-safe view set + Company Registry HTTP datasource + `create_hitl_task`) and `RESEARCH_WORKFLOW` (backoffice-only, five read-only `research-mcp` tools over the broader view set, no write tool of any kind). Same factory, two security envelopes.
...
- **`RESEARCH_WORKFLOW` reads through a dedicated MCP wrapper, not the Select AI Bridge.** `research-mcp` (`BACKOFFICE_AGENT_RO`) carries the flow's full read scope — five read-only `*_for_task` tools over the broader view set — without waiting on the policy corpus (`BACKLOG.md §5`) or on Select AI adoption (`BACKLOG.md §9`), and without reopening the SQL Query node's bind-variable hazard ([`issues/02`](../issues/02-sql-query-no-bind-variables.md)) that routing a live read path through the Select AI Bridge would put back in front of the flow.
