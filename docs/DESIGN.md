...
8. OPA MCP evaluates eligibility, AML, KYC, fair-lending pre-flight, escalation. Each tool output (allow / deny / warn) is captured as evidence for the recommendation; **deny does not short-circuit to a decision** — it becomes a strong signal toward the `DECLINE` tier on the recommendation packet.
