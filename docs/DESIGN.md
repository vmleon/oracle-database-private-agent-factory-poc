...
8. Select AI RAG retrieves policy citations relevant to the signals observed ([`BACKLOG.md §6`](../BACKLOG.md)); the OPA rate card returns an indicative band into the evidence packet for every application it can price, so the reviewer sees the figure the offer would rest on.
