...
| Read-scope guardrail (`RESEARCH_WORKFLOW`) | `BACKOFFICE_AGENT_RO` holds `SELECT` only on the `research_v_*` view set and `cust_360`, no `EXECUTE` anywhere and no write grant of any kind; `research-mcp`'s five tools each take only a task id and resolve it server-side. Still read-only; no base tables.                                                                                                                                                                                                                     |
