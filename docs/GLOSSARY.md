...
- **`score_caution_band_upper`** — the top of a "be careful" zone just above the floor; applicants in this band aren't rejected but can't be auto-approved either — a human reviews them. _Example: floor = 600, caution upper = 670 → a 640 applicant goes to manual review._
- **`auto_approve_amount_cap`** — the largest loan amount the system will auto-approve; bigger loans always go to a human reviewer. _Example: cap = $10,000. A $8,000 clean profile auto-approves; a $15,000 clean profile still goes to review._
...
- **APPROVE / REJECT / REFER_HUMAN** — the three outcomes the decisioning agent can produce. _Example: clean profile → APPROVE with a priced offer; DTI too high → REJECT; borderline score → REFER_HUMAN._
- **Reason code** — a short, standardised label explaining why an application was declined or referred. _Example: `DTI_OVER_CAP`, `EXPIRED_ID`, `SCORE_BELOW_FLOOR` — so two declines for "the customer was too indebted" appear identically in audit and customer notices._
