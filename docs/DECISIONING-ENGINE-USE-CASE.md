...
CREATE TABLE decision_audit (
  ...
  agent_run_id  VARCHAR2(60),    -- CHAT_AGENT runs only
  step_no       NUMBER,
...
CREATE TABLE research_audit (
  audit_id      NUMBER PRIMARY KEY,
  research_run_id VARCHAR2(60),  -- RESEARCH_AGENT runs only
...
# Outputs (allow / deny / warn / caution) feed CHAT_AGENT's recommendation tier;
...
- **OPA MCP server** wraps OPA's `/v1/data/...` HTTP endpoints as typed MCP tools, wired to `CHAT_AGENT` only.
...
- **Policy retrieval** — `search_policy(reasons)` → top-N policy chunks → cited in the recommendation reasoning. Available to both `CHAT_AGENT` and `RESEARCH_AGENT`.
- **Case retrieval** — `search_similar_cases(applicant_features)` → top-N similar past cases. `CHAT_AGENT` uses this to anchor the reasoning; `RESEARCH_AGENT` uses it at higher `k` and with deeper filters (date range, outcome, reviewer, similarity threshold) to support reviewer interrogation.
- **Hybrid retrieval** — vector + SQL filter (e.g., `product_type = 'PERSONAL_LOAN'`) for precision.

### What RAG is NOT used for

Not for the decision itself — that's the human reviewer's call, informed by OPA outputs + recommendation packet. RAG grounds the agent reasoning text and powers `RESEARCH_AGENT`'s answers when a reviewer asks _"what does the lending policy say about X?"_ or _"show me how we decided similar cases last year"_.
...
## `CHAT_AGENT` — customer-facing Private Agent Factory flow
...
### `RESEARCH_AGENT` — backoffice-only research flow
...
| `query_decision_audit`      | SQL (Select AI)          | `decision_audit` + `research_audit`              | Show prior `CHAT_AGENT` tool-call traces for similar applications               |
...
```
You are CHAT_AGENT for retail loan applications. You drive the chat, collect the
...
### `RESEARCH_AGENT` instructions (sketch)
...
### Per-application `CHAT_AGENT` tool-call sequence
...
1. **Per-tool audit** — `decision_audit` captures every `CHAT_AGENT` tool call (input, output, duration, status). `research_audit` captures every `RESEARCH_AGENT` interaction with the same shape, plus the `hitl_task_id` the reviewer was looking at and the reviewer's identity. Both are replayable.
...
4. **Replay** — backoffice UI surfaces the full agent trail for any application in chronological order; reviewers can re-run `CHAT_AGENT` against the stored audit input to verify the recommendation is reproducible, and re-run `RESEARCH_AGENT` against a point-in-time snapshot to verify a research answer.
...
- `CHAT_AGENT` calls `lookup_pricing` → returns `{ rate_value, term, amount, expiry }` as **indicative pricing** inside the recommendation packet so the reviewer can see what the offer would look like if they approve.
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Purpose limitation   | All tables tagged with a `purpose` policy in `system_config`; agents reject tool calls outside their registered purposes. `CHAT_AGENT` and `RESEARCH_AGENT` have distinct purpose sets.                                                                       |
| Data minimization    | Select AI tools target **views**, not raw tables. `CHAT_AGENT` uses the narrower customer-safe view set; `RESEARCH_AGENT` uses the broader backoffice view set. Neither agent ever sees base tables.                                                          |
| Access control       | Out of scope at the edge for the PoC (mock login picks the active user/role). Data-layer RLS/VPD on `customer_id` and role-scoped backoffice views are the production target once real auth is wired in. `RESEARCH_AGENT` is gated by the backoffice surface. |
| Sensitive attributes | `customer_protected_attrs` separated from `customer`; access logged separately; never sent to the `CHAT_AGENT` LLM. `RESEARCH_AGENT` can access them only when the reviewer explicitly requests a fairness-related research query.                            |
...
| POST   | `/v1/hitl/tasks/{id}/research`          | Send a message to `RESEARCH_AGENT` in the context of this task (threaded by `roomId`); returns the agent's reply + cited evidence  |
...
| POST   | `/v1/audit/{decision_id}/replay-chat`   | Re-run `CHAT_AGENT` against the stored audit input, diff the recommendation packets                                                |
| GET    | `/v1/research/conversations`            | Recent `RESEARCH_AGENT` conversations by reviewer, for replay / audit                                                              |
...
- **HITL Task Detail** — for an open task, shows:
  ...
  - A **Case Research Agent** chat panel — a conversational surface backed by `RESEARCH_AGENT`, scoped to this task's context. The reviewer can ask things like _"show me similar cases in the last 12 months with DTI > 0.4 and self-employed"_, _"what does our policy say about expat residency proof"_, _"when did dti_hard_cap last change and what was the prior value"_. Every reply cites its sources.
...
- (c) every expected `CHAT_AGENT` tool call appears in `decision_audit`,
...
| 23  | Reviewer asks Case Research Agent for similar cases                   | n/a                            | n/a                     | `RESEARCH_AGENT` returns top-N from `case_history` filtered by features; reply cites case_id + outcome + reviewer; logged in `research_audit`                          |
...
13. **Recommendation replay** — pick any decision; click "Replay `CHAT_AGENT`" → identical recommendation packet from stored audit input.
