...
| LLM + embeddings    | vLLM containers on a self-hosted GPU host (`Qwen/Qwen2.5-32B-Instruct-AWQ` + `BAAI/bge-m3`) — see [`DESIGN.md §11`](DESIGN.md#11-locked-decisions) | Reasoning + rationale + embeddings                                                                                                                                                                                                                                                |
...
1. **End-to-end test the parameterised `customer_id` + `application_id` wiring in `CHAT_WORKFLOW`'s SQL Query.** Both IDs are now fed via a Prompt (SQL Builder) node that inlines them into the SQL string — PAF's SQL Query node ignores `:name` bind variables (see [`issues/sql-query-no-bind-variables.md`](../issues/sql-query-no-bind-variables.md)). Once the Application Service exists (item 6) it threads both IDs from the session through the same flow inputs. Wiring detail: [`paf/flows/CHAT_WORKFLOW.md §Wiring the flow inputs`](../paf/flows/CHAT_WORKFLOW.md#wiring-the-flow-inputs).
