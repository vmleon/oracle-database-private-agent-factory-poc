...
- **Caddy HTTPS proxy + Oracle SSL wallet (local)**: standing infrastructure for any future HTTPS-from-DB work (OCI GenAI when sanctioned, RAG embedding endpoints, etc.). The compose includes a `caddy-ollama-tls` service (legacy name — its upstream is now the LAN vLLM endpoint, not Ollama) that terminates TLS on `:443` and reverse-proxies `/v1/*` to the configured LLM host (`manage.py local up` exports `OLLAMA_HOST`/`OLLAMA_PORT` from `VLLM_HOST`/`VLLM_GEN_PORT` as a compose compatibility shim until the compose file's substitution keys are renamed). `manage.py local up` (a) pre-generates a self-signed CA + leaf cert via `openssl` into `deploy/podman/caddy/tls/` (gitignored, persists across `down --purge`), (b) creates an Oracle SSL wallet at `/opt/oracle/dcs/commonstore/wallets/ssl/` with the Caddy CA trusted, (c) registers the wallet via the `SSL_WALLET` database property so `DBMS_CLOUD` finds it. Validated: `UTL_HTTP` over HTTPS from any session reaches the LLM endpoint through Caddy and returns 200.
- **Select AI on local is intentionally NOT wired (only on cloud / ADB)**: `manage.py local up` skips the `DBMS_CLOUD_AI` profile creation, drops any leftover credential / profiles (`VLLM_CRED` and the historical `OLLAMA_CRED`), and prints a yellow warning. Reason: Oracle Database Free 26ai (23.26.x) layers three checks that progressively reject every variant of a custom-endpoint Select AI profile we've tried (originally tested against an Ollama backend; the same DB-side validator constraints apply to a vLLM backend):
...
- `DBMS_CLOUD` + `DBMS_CLOUD_AI` installed via `catcon.pl`. Caddy TLS terminator in front of the LAN LLM endpoint plus an Oracle SSL wallet trusting Caddy's CA (`SSL_WALLET` database property) — HTTPS-from-DB calls work end-to-end via `UTL_HTTP`. Network ACL grants `AGENT_FACTORY → caddy-ollama-tls:443` (the compose service kept its historical name; its upstream is whatever LLM `VLLM_HOST:VLLM_GEN_PORT` resolves to).
