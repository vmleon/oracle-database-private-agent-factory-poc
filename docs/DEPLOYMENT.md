...
- Exact PAF flow JSON for the production `DECISIONING_AGENT` DAG — decided once the banking + decisioning schema and the in-DB Select AI tools exist.
...
1. Liquibase changesets `002-app-banking.yaml` through `008-queues.yaml` (banking + decisioning tables, REPORTING views, AGENT_TOOLS packages, vector indexes, TxEventQ).
2. Select AI bootstrap: profile, NL2SQL object list over `REPORTING.*`, RAG vector index over `policy_corpus`.
3. OPA MCP and OCR MCP services under `src/ai/`; the production `DECISIONING_AGENT` flow that combines them with the In-DB tools.
4. Spring Boot Application Service and the two Angular UIs.
