...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `manage.py setup local`       | Checks host prereqs (`podman`, `ansible-playbook`, `liquibase`, `python>=3.11`), then prompts for Ollama host/port (default `host.containers.internal:11434`, override to LAN GPU / DGX Spark), OCR host/port, generative model (default `qwen2.5:7b-instruct`), embedding model (default `bge-m3`, 1024 dims), local DB password. Writes `.env` with `DEPLOYMENT_TARGET=local`. Errors with install hints if any prereq is missing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `.env`                                                  |
...
2. `python manage.py setup local` — interactive; prompts for Ollama host (default `host.containers.internal:11434`), OCR host, generative model (`qwen2.5:7b-instruct`), embedding model (`bge-m3` @ 1024 dims), local DB password. Writes `.env` with `DEPLOYMENT_TARGET=local`.
...
1. `CHAT_AGENT` flow in PAF — customer-safe, combines OPA MCP + OCR MCP + Company Registry datasource + HITL MCP. Sources `customer_id` / `application_id` from session context (not from the user) via an in-flow SQL Query node so `create_hitl_task` can be called as the agent's terminal action. Blueprint: [`paf/flows/CHAT_AGENT.md`](../paf/flows/CHAT_AGENT.md).
