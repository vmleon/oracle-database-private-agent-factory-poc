...
3. OPA MCP and OCR MCP services under `src/ai/`; the `CHAT_AGENT` flow that combines them with the in-DB `create_hitl_task` tool.
4. `RESEARCH_AGENT` flow — broader Select AI profile + RAG, no side-effect tools.
