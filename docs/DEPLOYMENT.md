...
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `manage.py setup local`       | Checks host prereqs (`podman`, `ansible-playbook`, `liquibase`, `python>=3.11`), then prompts for Ollama host/port (default `host.containers.internal:11434`, override to LAN GPU / DGX Spark), OCR host/port, generative model (default `llama3.3:70b-instruct-q4_K_M`), embedding model (default `bge-m3`, 1024 dims), local DB password. Writes `.env` with `DEPLOYMENT_TARGET=local`. Errors with install hints if any prereq is missing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `.env`                                                  |
...
| `manage.py paf bootstrap`     | Prints the PAF UI installer URL + a step-by-step cheatsheet: admin-user creation, DB connection values (host `oracle-free-26ai`, port `1521`, service `FREEPDB1`, user `AGENT_FACTORY`, password from `.env`), Production mode, the LLM Configuration values (Ollama host/port from `.env`, generative + embedding model IDs), the two Select AI profiles to register (`chat_profile` customer-safe + `research_profile` broader read-only), the OPA + OCR MCP server registrations (wired to `CHAT_AGENT` only), the **Company Registry HTTP datasource** registration (point PAF at `http://${REGISTRY_HOST}:${REGISTRY_PORT}/openapi.json`; wired to `CHAT_AGENT` only), and the three Agent Builder flows to import in order: `HELLO_AGENT` → `CHAT_AGENT` → `RESEARCH_AGENT`. Captures each agent's published run URL into `PAF_HELLO_RUN_URL`, `PAF_CHAT_RUN_URL`, `PAF_RESEARCH_RUN_URL`. Auto-resolves a `.local` Bonjour `OLLAMA_HOST` to its current LAN IP. Read-only — no UI automation. | stdout                                                  |
...
2. `python manage.py setup local` — interactive; prompts for Ollama host (default `host.containers.internal:11434`), OCR host, generative model (`llama3.3:70b-instruct-q4_K_M`), embedding model (`bge-m3` @ 1024 dims), local DB password. Writes `.env` with `DEPLOYMENT_TARGET=local`.
...
| `PAF_HELLO_RUN_URL`, `PAF_CHAT_RUN_URL`, `PAF_RESEARCH_RUN_URL`, `PAF_LOGIN_VALIDATION_URL` | Captured after `paf bootstrap` — one run URL per agent flow                                                                                                                                                      |
