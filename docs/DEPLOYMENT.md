...
- **TLS**: the public listener and the internal MCP listener each carry a self-signed certificate. PAF trusts the internal one through its administrator certificate store (`paf prepare`); the backend's `PafClientConfig` trusts the hop to PAF without verification, an exposure limited to one in-VCN hop.
