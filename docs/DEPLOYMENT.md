...
| `manage.py paf bootstrap`     | Prints the PAF UI installer URL + a step-by-step cheatsheet: admin-user creation, DB connection values (host `oracle-free-26ai`, port `1521`, service `FREEPDB1`, user `AGENT_FACTORY`, password from `.env`), Production mode, the LLM Configuration values (Ollama host/port from `.env`, generative + embedding model IDs), the two Select AI profiles to register (`chat_profile` customer-safe + `research_profile` broader read-only), the OPA + OCR + HITL MCP server registrations (wired to `CHAT_AGENT` only), the **Company Registry HTTP datasource** registration (point PAF at `http://${REGISTRY_HOST}:${REGISTRY_PORT}/openapi.json`; wired to `CHAT_AGENT` only), and the two Agent Builder flows to import in order: `CHAT_AGENT` → `RESEARCH_AGENT`. Captures each agent's published run URL into `PAF_CHAT_RUN_URL`, `PAF_RESEARCH_RUN_URL`. Auto-resolves a `.local` Bonjour `OLLAMA_HOST` to its current LAN IP. Read-only — no UI automation. | stdout                                                  |
...
- Three of four `CHAT_AGENT` tool channels are validated end-to-end against qwen2.5:7b through a development-time Agent Builder flow: OPA (`required_documents` → four-doc list), OCR (MARGINAL + UNUSABLE responses), and Company Registry (active / dormant / not-registered). `hitl-mcp.create_hitl_task` is validated independently via direct Python end-to-end against the DB (task row + queue message confirmed); agent-level wiring of it lands with the `CHAT_AGENT` flow build that uses the application context from a SQL Query node.

Next deliverables, in order:

1. Verify `CHAT_AGENT` against `qwen2.5:32b-instruct`, then parameterize `customer_id` in the flow's SQL Query (currently hardcoded — the highest-priority follow-up). Two paths: a PAF flow-input variable bound to the SQL Query's `:customer_id`, or threading it from the future Application Service. Build blueprint + open questions: [`paf/flows/CHAT_AGENT.md`](../paf/flows/CHAT_AGENT.md).
