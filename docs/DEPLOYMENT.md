...
`deploy/tf/app/` is the workload root module; `deploy/tf/iam/` is a second root holding the tenancy-level dynamic groups and Generative AI policy, applied once by an administrator because identity resources sit outside the workload compartment's permission scope. The four tiers differ only in shape and in the play they run, so they share one `deploy/tf/modules/tier/` module rather than a module each; `tier_name` is validated against the same four words used for the Ansible directories, artifact names and instance display names.
...
5. `cd deploy/tf/iam && terraform init && terraform apply` — dynamic groups and the Generative AI policy. Needs a tenancy-admin profile, and only ever runs once per compartment.
6. `cd deploy/tf/app && terraform init && terraform plan -out=tfplan && terraform apply tfplan`.
