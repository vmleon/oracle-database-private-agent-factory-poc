...
2. `python manage.py setup local` — interactive; prompts for vLLM host (the GPU host's hostname or IPv4), gen + embed ports (defaults `8000` / `8001`), generative model (`Qwen/Qwen2.5-72B-Instruct-AWQ`), embedding model (`BAAI/bge-m3` @ 1024 dims), OCR host, local DB password. Writes `.env` with `DEPLOYMENT_TARGET=local`.
...
- `CHAT_WORKFLOW` is built as a **two-agent pipeline** in PAF Agent Builder (`EvaluationAgent` → `RecommendationAgent`) and validated end-to-end against `Qwen/Qwen2.5-72B-Instruct-AWQ` served by vLLM on a self-hosted GPU host. All four tool channels exercised in one run: OPA MCP (`required_documents`, `evaluate_eligibility`), Company Registry HTTP datasource (`verify_employer`), HITL MCP (`create_hitl_task`). Full build blueprint with custom-instruction blocks for both agents: [`paf/flows/CHAT_WORKFLOW.md`](../paf/flows/CHAT_WORKFLOW.md). OCR MCP is registered with PAF but intentionally unwired pending the real pipeline.

Next deliverables, in order:

1. End-to-end test `CHAT_WORKFLOW` across the seeded scenarios. `customer_id` / `application_id` are resolved server-side from an opaque session token via `banking-mcp.lookup_application` (cx_Oracle bind variables, fail-secure) — never from the chat message — which is why a PAF SQL Query node wasn't viable ([`issues/01-sql-query-no-bind-variables.md`](../issues/01-sql-query-no-bind-variables.md)). A pytest harness covers the tiers plus the fail-secure and prompt-injection cases: [`tests/test_chat_workflow.py`](../tests/test_chat_workflow.py). Once the Application Service exists it mints the token at login and threads it through the same flow input. Add `OcrAgent` between the two agents when the OCR pipeline becomes real. Capture the workflow JSON to `paf/flows/chat_workflow.flow.json` as a reference snapshot — PAF has no export endpoint and re-import hard-fails on tool-ref drift ([`issues/05-no-flow-export-endpoint.md`](../issues/05-no-flow-export-endpoint.md)).
