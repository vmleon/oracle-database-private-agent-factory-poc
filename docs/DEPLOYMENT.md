...
| `manage.py info`                   | Prints the load balancer address, the per-path URLs, the bastion and the models.                                                                                                                                                                                                                           | —                                           |
...
- **TLS**: the public listener and the internal MCP listener each carry a self-signed certificate. PAF trusts the internal one through its administrator certificate store (`paf trust-ca`); the backend's `PafClientConfig` trusts the hop to PAF without verification, an exposure limited to one in-VCN hop.
