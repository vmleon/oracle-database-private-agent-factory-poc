...
3. [Install PAF, register the LLM through its UI wizard, and raise the agent iteration budget.](#3-install-paf)
...
Download the ARM64 PAF tarball from Oracle first (~2.4 GB) so you can point `setup local` at it.
...
Six post-install registrations in the PAF admin area — five MCP servers and one HTTP datasource. (`CHAT_FLOW` needs no Database datasource — the agents read through `banking-mcp.get_context`, not a SQL Query node; see §4b.) All target the `CHAT_FLOW` flow; the `RESEARCH_WORKFLOW` flow has no external tools by design.

> ⚠️ **Run both once before any registration below — every install, including after `--purge`:**
>
> ```bash
...
> ```
>
> `allow-internal-mcp` clears PAF's private-network guard. Skip it and the **first** MCP server (or the Company Registry datasource) fails with _"400 Bad Request: MCP server URL resolves to a private or non-routable network address and is not allowed"_.
>
> `trust-ca` registers the `mcp-proxy` gateway certificate in PAF's administrator certificate store, which is what PAF's outbound HTTP client verifies against. Skip it and **every** MCP server fails its connection test with _"Could not connect to the remote MCP server. Make sure it is running and reachable."_ — a TLS-verification failure, not a reachability one: the containers are healthy and the route works.
>
> Both settings live in PAF's install state and **reset on every reinstall**, so re-run them after each fresh install.
...
> **MCPs go through the `mcp-proxy` TLS gateway.** PAF rejects `http://` MCP URLs and blocks private-network URLs by default. So the MCP apps are fronted by a Caddy TLS gateway (`mcp-proxy:8443`, self-signed cert generated by `local up`), and two settings are applied once per install: `python manage.py paf trust-ca` puts the gateway certificate in PAF's administrator certificate store so the TLS handshake verifies, and `python manage.py paf allow-internal-mcp` relaxes the private-network guard. URLs below are the gateway routes, **not** the raw `http://<svc>:<port>` (those still exist internally; the gateway forwards to them).
...
| `banking-mcp`     | `https://mcp-proxy:8443/banking/mcp/`     | six read-only tools, all token-keyed: `get_context(session_token)` (the universal read the origination agents use — customer + application + profile + credit + `missing[]`/staleness), `lookup_application(session_token)`, `evaluate_eligibility_for_session(session_token)`, `required_documents_for_session(session_token)`, `verify_employer_for_session(session_token)`, and `hitl_status_for_session(session_token)` |
