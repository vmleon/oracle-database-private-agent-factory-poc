...
1. `APP.auth_session(session_token PK, customer_id, application_id, expires_at)` — one row per test scenario, seeded by Liquibase changeset 011. Tokens are deterministic + readable for ergonomics (`paf-test-alice-1`, `paf-test-david-3`, etc.).
