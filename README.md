...
- Three of four `CHAT_AGENT` tool channels are validated end-to-end against qwen2.5:7b through a development-time Agent Builder flow: OPA (`required_documents`), OCR (`extract_document` for MARGINAL/UNUSABLE scenarios), and Company Registry (`verify_employer` for active/dormant/unknown). The fourth channel — `hitl-mcp.create_hitl_task` — is validated via direct Python end-to-end against the DB; agent-level wiring lands with the `CHAT_AGENT` flow build.

What's next, in order:

1. **`CHAT_AGENT` flow** in PAF — customer-facing, combines OPA MCP + OCR MCP + Company Registry datasource + HITL MCP. Sources `customer_id` / `application_id` from session context (in-flow SQL Query node) instead of the user typing it, then calls `create_hitl_task` as the terminal action. Blueprint: [`paf/flows/CHAT_AGENT.md`](paf/flows/CHAT_AGENT.md).
