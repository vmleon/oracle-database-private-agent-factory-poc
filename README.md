...
- `CHAT_WORKFLOW` is a **two-agent pipeline** (`EvaluationAgent` → `RecommendationAgent`) running on `Qwen/Qwen2.5-72B-Instruct-AWQ` via a self-hosted vLLM endpoint. `EvaluationAgent` has read-only tools — `required_documents` (opa-mcp), `verify_employer` (Company Registry REST), `evaluate_eligibility` (opa-mcp) — and emits a structured evidence block. `RecommendationAgent` reads that evidence, decides `APPROVE` / `REVIEW` / `DECLINE`, and calls `hitl-mcp.create_hitl_task` exactly once (the only side-effect tool in the workflow). Build blueprint with full custom-instructions blocks: [`paf/flows/CHAT_WORKFLOW.md`](paf/flows/CHAT_WORKFLOW.md). `ocr-mcp` is registered with PAF but unwired pending the real OCR pipeline (`OcrAgent` slots in between the existing two when it lands).

  ![CHAT_WORKFLOW in PAF Agent Builder](images/CHAT_WORKFLOW.png)

What's next, in order:

1. **End-to-end test `CHAT_WORKFLOW` across the seeded scenarios.** `customer_id` / `application_id` are never taken from the chat message — they're resolved server-side from an opaque session token via `banking-mcp.lookup_application` (cx_Oracle bind variables, fail-secure), which is why a PAF SQL Query node wasn't viable (see [`issues/01-sql-query-no-bind-variables.md`](issues/01-sql-query-no-bind-variables.md)). A pytest harness covers the six happy-path tiers plus the fail-secure and prompt-injection cases: [`tests/test_chat_workflow.py`](tests/test_chat_workflow.py). Scenario table + seeded tokens in [`paf/flows/CHAT_WORKFLOW.md §Test prompts`](paf/flows/CHAT_WORKFLOW.md#test-prompts).
