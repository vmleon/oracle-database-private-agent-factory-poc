...
- `CHAT_WORKFLOW` is a **two-agent pipeline** (`EvaluationAgent` → `RecommendationAgent`) built in PAF Agent Builder and validated end-to-end against `Qwen/Qwen2.5-32B-Instruct-AWQ` on a self-hosted vLLM endpoint (NVIDIA DGX Spark / GB10). `EvaluationAgent` calls three read-only tools — `required_documents` (opa-mcp), `verify_employer` (Company Registry REST), `evaluate_eligibility` (opa-mcp) — and emits a structured evidence block; `RecommendationAgent` reads the evidence, decides `APPROVE` / `REVIEW` / `DECLINE`, and calls `hitl-mcp.create_hitl_task` exactly once (the only side-effect tool in the whole workflow). The split was driven by failures on a single-agent prototype where the LLM batched all four tool calls into one response and fabricated intermediate results — see [`paf/flows/CHAT_WORKFLOW.md §Lessons`](paf/flows/CHAT_WORKFLOW.md). `ocr-mcp` is registered with PAF but intentionally unwired pending the real OCR pipeline (`OcrAgent` slots in between the existing two).
