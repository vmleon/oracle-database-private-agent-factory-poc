...
1. **Parameterise `customer_id` and `application_id` in `CHAT_WORKFLOW`'s SQL Query** — wired via a Prompt (SQL Builder) node that inlines `{{customer_id}}` / `{{application_id}}` into the SQL string (PAF's SQL Query node ignores `:name` bind variables, see [`issues/01-sql-query-no-bind-variables.md`](issues/01-sql-query-no-bind-variables.md)). Two Text Input nodes feed the Prompt; the rendered SQL feeds the SQL Query node's `query` input port. The combined filter is the production-correct authorization shape and matches `chat_message`'s `(room_id, customer_id, application_id)` key. Doc + wiring in [`paf/flows/CHAT_WORKFLOW.md §Wiring the flow inputs`](paf/flows/CHAT_WORKFLOW.md#wiring-the-flow-inputs). Pending end-to-end test in Playground across the five scenarios.
