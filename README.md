...
- OPA + the three MCP wrappers as systemd units on the `backend` compute, behind the private TLS load balancer, one per database identity: `banking-mcp` (`CUSTOMER_AGENT_RO`) serves the deterministic `*_for_session` reads and calls OPA server-side; `application-mcp` (`CUSTOMER_AGENT_RW`) writes the `DRAFT` application (`upsert_application`) and files the recommendation packet (`create_hitl_task`, wrapping the in-DB `BANK_TOOLS.PKG_AGENT_TOOLS.create_hitl_task`); `research-mcp` (`BACKOFFICE_AGENT_RO`) serves five read-only `*_for_task` tools over the case under review, comparable cases, decision history, transactions and policy changes.
...
- `RESEARCH_WORKFLOW` is **one Agent node holding no tools and no sub-agents**, fed by **five deterministic `research-mcp` nodes** off one wired task id — `recommendation_for_task`, `similar_cases_for_task`, `decision_history_for_task`, `transactions_for_task` and `policy_changes_for_task`. A gate closes the run with an "unavailable" message when the task doesn't resolve; otherwise the agent reads all five results and writes a four-section case file that never states a lean, and `ResearchSummary.screen` in the Application Service enforces that server-side before the summary reaches the reviewer or `BANK_CORE.research_summary`. The reviewer triggers a run from the task detail screen through `POST /v1/research/tasks/{taskId}/run`; `GET /v1/research/tasks/{taskId}` returns the stored summary so reopening a case doesn't re-run the agent, and each tool call is audited to `BANK_CORE.research_audit` via `POST /v1/audit/research`, keyed by `research_run_id`. Load it by importing [`paf/flows/RESEARCH_WORKFLOW.paf`](paf/flows/RESEARCH_WORKFLOW.paf) (password `WelcomeAmigo123!`) or by building it from the blueprint: [`paf/flows/RESEARCH_WORKFLOW.md`](paf/flows/RESEARCH_WORKFLOW.md). Either way see [`CLOUD.md §10`](CLOUD.md#10-load-research_workflow).
...
3. **Narrow the customer read path's session lookup.** `CUSTOMER_AGENT_RO` can select every row of `auth_session`, so a prompt injection against the chat agent reaches more than the one token it was handed. [`BACKLOG.md §3`](BACKLOG.md).
4. **One source of truth for the thresholds**, so parameter history stops being a table that never receives a row. [`BACKLOG.md §4`](BACKLOG.md).
5. **Policy retrieval and the citations it would produce.** `policy_corpus` exists and is empty. [`BACKLOG.md §5`](BACKLOG.md).

Then: document collection and the fair-lending producer — [`BACKLOG.md §6`–`§7`](BACKLOG.md).

Two known constraints not in the "next" list because they're decided, and one hop waiting on a spike:

- **Select AI is parked.** `CHAT_FLOW` reads through the `banking-mcp` wrappers; the database is registered as a data source and the grants are in place, but no flow node uses Select AI. Adopting it is a flow redesign, tracked in [`BACKLOG.md §9`](BACKLOG.md).
- **The load balancer's hop to PAF is unverified.** Encrypted but unauthenticated, and it is the only backend set that speaks TLS at all — the rest of the VCN is plaintext by design. Closing it needs a second apply and a spike on OCI's certificate matching, tracked in [`BACKLOG.md §8`](BACKLOG.md).
