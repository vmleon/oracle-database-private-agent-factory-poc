...
- Liquibase changelog `001-012`: users + grants, banking core (incl. employment / transactions / bureau / facilities), decisioning + HITL + chat persistence, `system_config` + parameter history, REPORTING view sets, `AGENT_TOOLS` PL/SQL package (`create_hitl_task`, `upsert_draft_application`), vector RAG tables (`policy_corpus`, `case_history`), TxEventQ queues (`HITL_REQUEST`, `OCR_REQUEST`, `OCR_EXCEPTION_Q`), and scenario seed customers + `case_history` rows for the test bench. Changeset 011 adds the opaque-session table + seed tokens; 012 makes draft fields nullable and seeds a no-application customer for intake.
...
1. **End-to-end test `CHAT_WORKFLOW` across the seeded scenarios.** `customer_id` / `application_id` are never taken from the chat message — they're resolved server-side from an opaque session token via `banking-mcp.get_context` (cx_Oracle bind variables, fail-secure), which is why a PAF SQL Query node wasn't viable (see [`issues/01-sql-query-no-bind-variables.md`](issues/01-sql-query-no-bind-variables.md)). A pytest harness covers the six happy-path tiers plus the fail-secure and prompt-injection cases: [`tests/test_chat_workflow.py`](tests/test_chat_workflow.py). Scenario table + seeded tokens in [`paf/flows/CHAT_WORKFLOW.md §Test prompts`](paf/flows/CHAT_WORKFLOW.md#test-prompts).
2. **Add `OcrAgent` between `Docs & Employer` and `Eligibility`** when the real OCR pipeline lands. `Docs & Employer` already emits the required-document list; `OcrAgent` extracts each via `ocr-mcp.extract_document` and appends quality findings to the evidence block.
3. **Capture the workflow JSON** to `paf/flows/chat_workflow.flow.json` as a reference snapshot — PAF has no export endpoint and re-import hard-fails on tool-ref drift, so a clean redeploy still rebuilds the canvas by hand and diffs against the snapshot ([`issues/05-no-flow-export-endpoint.md`](issues/05-no-flow-export-endpoint.md)).
...
5. **Spring Boot Application Service (shipped, Plans 1–2)** — mints the opaque session token at login, lists customers (incl. no-application ones), brokers each chat turn, and supports conversational intake (`get_context` / `upsert_application`). Remaining: document uploads (enqueue `OCR_REQUEST`), writing the Blockchain `decision` row at HITL close, and the two Angular UIs (customer chat, backoffice with Case Research Agent panel).
6. **Customer 360 view** — finish `REPORTING.cust_360` joining demographics, balances, products held, recent transactions, bureau snapshot, employer-verification. Feature source for item 7. See [`BACKLOG.md §2`](BACKLOG.md#2-customer-360-curated-view).
...
8. **Product-recommendation workflow** — second PAF Agent Builder flow over `REPORTING.cust_360`, mirroring the `CHAT_WORKFLOW` pattern; consumes the credit-score tool from item 7 as one of its signals. See [`BACKLOG.md §1`](BACKLOG.md#1-proactive-product-recommendation-as-a-second-workflow).
9. **TOON feasibility spike** — confirm whether a PAF Function node can run a `toon` library to transform tool output, or whether encoding has to happen in the prompt-builder outside PAF. Independent — can happen in parallel. See [`BACKLOG.md §4`](BACKLOG.md#4-toon-feasibility-spike).
