...
2. **Add `OcrAgent` between `Docs & Employer` and `Recommendation`** when the real OCR pipeline lands. `Docs & Employer` already emits the required-document list; `OcrAgent` extracts each via `ocr-mcp.extract_document` and appends quality findings to the evidence block.
...
5. **Document uploads + Case Research panel** — a chat upload endpoint that stores the file and enqueues `OCR_REQUEST` (feeds item 2), and the Case Research Agent panel in the reviewer portal once `RESEARCH_WORKFLOW` (item 4) exists.
